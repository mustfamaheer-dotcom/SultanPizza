...
function getIcon(name) { for (const [k, v] of Object.entries(foodIcons)) { if (name.includes(k)) return v }; return '🍕' }
...
const MenuItem = React.forwardRef(({ item, index }, ref) => {
  return (
    <motion.div ref={ref} layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
      transition={{ duration: 0.4, delay: index * 0.03 }} whileHover={{ y: -6 }}
      style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 20, padding: 0, cursor: 'default', overflow: 'hidden', transition: 'all 0.4s ease' }}
      ...
      <div style={{ padding: '22px 24px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--text)', marginBottom: 1 }}>{item.name}</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', direction: 'ltr', textAlign: 'left' }}>{item.en}</div>
            </div>
          </div>
          <span style={{ fontSize: '0.6rem', padding: '4px 14px', borderRadius: 20, background: item.cat === 'italian' ? 'rgba(196,30,58,0.15)' : 'rgba(255,215,0,0.12)', color: item.cat === 'italian' ? 'var(--red)' : 'var(--gold)', fontWeight: 700, whiteSpace: 'nowrap' }}>
            {item.cat === 'italian' ? 'إيطالي' : 'شرقي'}
          </span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginTop: 16, padding: '12px 0 0', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
          {[{ size: 'صغير', en: 'S', key: 'small' }, { size: 'وسط', en: 'M', key: 'medium' }, { size: 'كبير', en: 'L', key: 'large' }, { size: 'عائلي', en: 'F', key: 'family' }].map((s) => (
            <div key={s.key} style={{ textAlign: 'center', padding: '6px 4px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', transition: 'background 0.3s ease' }}
              onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,215,0,0.06)'}
              onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-dim)', fontWeight: 600, marginBottom: 2 }}>{s.size}</div>
              <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--gold)', lineHeight: 1.3 }}>{item[s.key]}</div>
...
export default function Menu() {
  ...
  return (
    <section id="menu" className="section" style={{ background: 'linear-gradient(180deg, var(--dark) 0%, var(--surface) 50%, var(--dark) 100%)' }}>
      ...
      <motion.div layout style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 18, maxWidth: 1400, margin: '0 auto' }}>
